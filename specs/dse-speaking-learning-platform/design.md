...
Onboarding 使用「一題、一個麥克風、一個下一步」的聲音優先版面。桌面左側只解釋三步：看題、直接回答、回聽改善；右側展示一條真實 DSE Speaking 題目、三段式答題提示和大型圓形錄音操作。手機首屏必須同時看見題目與「開始說」，不能先出現問卷、文字欄或產品導覽。

錄音完成後只推薦第一節教學，並明確說明推薦不是能力評分。錄音失敗時才顯示恢復指引；若此入口不提供文字後備，必須提供可進入第一課的非阻塞路徑。目標年份、目標 Level、信心與弱項只能在第一次開口後作可選個人化設定，不能以預設值或推斷結果代替學生回答。

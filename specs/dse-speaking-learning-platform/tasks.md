...
- [x] 13. 將所有首次操作改為聲音優先
  - 以單題錄音取代四步能力問卷；不建立虛構 learner profile。
  - 移除 11 節課「先輸入 12 個英文詞」的硬門檻，只有麥克風失敗後才開放文字後備。
  - IR 準備筆記改為預設收起的可選工具；GD 每一輪都要求新的錄音，不能沿用上一輪逐字稿。
  - 新錄音與重錄會清除舊逐字稿、自檢及分析狀態，防止舊證據被提交為新回答。
  - 以單元測試、手機與桌面瀏覽器結構檢查驗證「先說、後回看」流程。
  - _Requirement: R1, R2, R4, R5, R6, R7, R9, R11_

## 驗證備註

- 2026-08-27 聲音優先重構通過 9 個 Vitest 檔案（25 項）、TypeScript、ESLint 及 production build；390×844 手機檢查確認 onboarding 首屏 0 個輸入欄、可直接看見「開始說」，IR／GD 初始狀態亦無可見文字欄或橫向溢出。為避免代替使用者授權敏感權限，本輪沒有在瀏覽器按下麥克風；真實錄音、轉寫與 AI provider 的既有生產驗收記錄仍與本次 UI 回歸分開判定。

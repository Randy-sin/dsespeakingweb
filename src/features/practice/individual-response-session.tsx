  ...
  return (
    <main id="main-content" className="mx-auto max-w-[1200px] px-4 py-10 sm:px-7 lg:px-10">
      ...
      <div className="mt-9 grid gap-8 lg:grid-cols-[1fr_360px]">
        <section>
          <p className="eyebrow text-[#c84b31]">{lesson.englishTitle}</p>
          <h1 className="mt-5 font-serif text-4xl leading-tight sm:text-5xl">{lesson.prompt}</h1>
          <div className="mt-8 border-l border-[#c84b31] pl-5"><p className="eyebrow text-[#8a8175]">Answer framework</p><p className="mt-3 text-sm leading-7 text-[#6d695f]">{lesson.steps.join(" → ")}</p></div>
        ...
        <aside className="space-y-4">
          <div className="paper-surface paper-rule p-6"><Lightbulb className="h-5 w-5 text-[#c84b31]" /><p className="mt-5 font-serif text-2xl">說完後自我檢查</p><ul className="mt-5 space-y-3 text-sm leading-6 text-[#6d695f]">{lesson.steps.slice(0, 4).map((step) => <li key={step}>— {step}</li>)}</ul></div>
          <div className="border border-[#bdb3a2] p-5"><p className="flex items-center gap-2 text-sm font-semibold"><Clock3 className="h-4 w-4" />請保持自然語速</p><p className="mt-2 text-xs leading-5 text-[#8a8175]">在一分鐘內清楚完成三個重點，比塞入更多但未解釋的內容更有效。</p></div>
        </aside>
      </div>
    </main>
  );

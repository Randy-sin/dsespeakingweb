...
export async function POST(request: NextRequest) {
  try {
    ...
    const savedSessionId = await saveDiscussionTurns({ supabase, userId: user.id, mode: "group-discussion", task: context, transcript: learnerTurn, sessionId }, response);
    return NextResponse.json({ ok: true, response, sessionId: savedSessionId, persisted: true, latencyMs: result.latencyMs });
  } catch (error) {

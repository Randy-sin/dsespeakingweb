  ...
  return (
    <main id="main-content" className="mx-auto max-w-[1440px] px-4 py-10 sm:px-7 lg:px-10 lg:py-16">
      ...
      <div className="mt-10 grid gap-12 lg:grid-cols-12">
        <aside className="lg:col-span-2"><p className="eyebrow text-[#ad3f29]">Lesson {lesson.number}</p><div className="mt-5 h-px bg-[#bdb3a2]" /><p className="mt-4 flex items-center gap-2 font-mono text-[11px] text-[#665f55]"><Clock3 className="h-3.5 w-3.5" />{lesson.duration} MINUTES</p><p className="mt-3 font-mono text-[11px] text-[#665f55]">SKILL · {lesson.skill.toUpperCase()}</p></aside>
        <article className="lg:col-span-7">
          <p className="font-serif text-lg italic text-[#665f55]">{lesson.englishTitle}</p>
          ...
          <section className="mt-14 border-t border-[#bdb3a2] pt-9">
            ...
            <ol className="mt-7 grid gap-px border border-[#bdb3a2] bg-[#bdb3a2] sm:grid-cols-2">{lesson.steps.map((step, index) => <li key={step} className="flex min-h-28 gap-4 bg-[#faf7ef] p-5"><span className="font-mono text-xs text-[#ad3f29]">0{index + 1}</span><p className="font-serif text-xl leading-7">{step}</p></li>)}</ol>
          </section>
        ...
        <aside className="lg:col-span-3"><div className="sticky top-24 border-l border-[#ad3f29] pl-5"><p className="eyebrow text-[#ad3f29]">Teacher&apos;s margin</p><p className="mt-4 text-sm leading-7 text-[#6d695f]">先把結構練穩，再改善句式。第一次練習只需要清楚完成每一步，不需要追求艱深詞彙。</p></div></aside>
      </div>
    </main>
  );
}

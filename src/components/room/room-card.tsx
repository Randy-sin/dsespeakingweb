...
export function RoomCard({ room }: RoomCardProps) {
  ...
  const statusConfig: Record<string, { label: string; color: string; dot: string }> = {
    waiting: { label: "等待中", color: "text-emerald-600 bg-emerald-50 border-emerald-100", dot: "bg-emerald-500" },
    preparing: { label: "准备中", color: "text-amber-600 bg-amber-50 border-amber-100", dot: "bg-amber-500" },
    discussing: { label: "讨论中", color: "text-blue-600 bg-blue-50 border-blue-100", dot: "bg-blue-500" },
    individual: { label: "回应中", color: "text-violet-600 bg-violet-50 border-violet-100", dot: "bg-violet-500" },
    finished: { label: "已结束", color: "text-neutral-500 bg-neutral-100 border-neutral-200", dot: "bg-neutral-400" },
  };
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start justify-between mb-4">
        <span className={`inline-flex items-center gap-1.5 text-[11px] font-medium px-2.5 py-1 rounded-full border ${status.color}`}>
          <span className={`w-1.5 h-1.5 rounded-full ${status.dot} ${room.status === "waiting" ? "animate-pulse" : ""}`} />
          {status.label}
        </span>
        ...
        <div className="flex items-center gap-1.5 mb-3">
          ...
          <span className="text-[12px] text-neutral-500">
            {(() => {
              ...
              const prefix = isToday(d) ? "今天" : isTomorrow(d) ? "明天" : format(d, "M/d", { locale: zhCN });
      ...
      <div className="flex items-center justify-between pt-4 border-t border-neutral-50">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center -space-x-2">
            {room.room_members?.slice(0, 4).map((member, i) => (
              <Avatar
                key={member.id}
            ...
            {memberCount < room.max_members && (
              <div className="h-7 w-7 rounded-full border-2 border-white bg-neutral-50 flex items-center justify-center border-dashed border-neutral-200" style={{ zIndex: 0 }}>
